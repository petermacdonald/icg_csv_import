...
function icg_csv_import_menu() {
  $items['admin/content/csv_import'] = array(
    ...
    'page arguments' => array('icg_csv_import_form'),
//  'file' => 'icg_csv_import.admin.inc',
  );
  return $items ;
}
...
function icg_csv_import_form() {
  ...
  $form['#validate'] = array(
    'icg_csv_import_validate_fileupload',
    'icg_csv_import_form_validate',
  ) ;
  return $form ;
}
 ...
 */
function icg_csv_import_validate_fileupload(&$form, &$form_state) {
  $validators = array(
  ...
  if ( $file = file_save_upload('csvfile', $validators, 'temporary://') ) {
    ...
    if ($directory_exists) {
      ...
      if (file_copy($file, $destination, FILE_EXISTS_REPLACE)) {
        ...
        form_set_error('icg_csv_import', t('Unable to copy upload file to !dest', array('!dest' => $destination)));
      }
    }
  }
}
 ...
 * expect it to.
 */
function icg_csv_import_form_validate(&$form, &$form_state) {
  if ( isset( $form_state['values']['csvupload'] ) ) {
    if ( $handle = fopen($form_state['values']['csvupload'], 'r') ) {
      ...
      if ( $line = fgetcsv($handle, 4096) ) {
         ...
         */
        // if ( $line[0] != 'Example CSV for icg_csv_import.module - http://github.com/xurizaemon/icg_csv_import' ) {
...
function icg_csv_import_form_submit(&$form, &$form_state) {
  ...
  if ( isset( $form_state['values']['csvupload'] ) ) {
    if ( $handle = fopen($form_state['values']['csvupload'], 'r') ) {
      $batch['operations'][] = array('_icg_csv_import_remember_filename', array( $form_state['values']['csvupload'] ) ) ;
      $line_count = 1 ;
      $first = TRUE ;
      ...
      while ( $line = fgetcsv($handle, 4096) ) {
         ...
         */
        $batch['operations'][] = array('_icg_csv_import_import_line', array(array_map('base64_encode', $line)));
      }
      fclose($handle);
    } // we caught this in icg_csv_import_form_validate()
  } // we caught this in icg_csv_import_form_validate()
  batch_set($batch);
}
 ...
 */
function _icg_csv_import_remember_filename($filename, &$context) {
 ...
 */
function _icg_csv_import_import_line($line, &$context) {
